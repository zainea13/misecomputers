...
def process_payment():
    """Process the payment"""
    if request.vars.stripeToken:
        try:
            # Create charge
            charge = stripe.Charge.create(
                amount=int(request.vars.amount),
                currency=request.vars.currency,
                source=request.vars.stripeToken,
                description='Order payment'
            )
            
            # Payment successful
            session.flash = 'Payment successful!'
            redirect(URL('payment_success'))
            
        except stripe.error.CardError as e:
            # Card declined
            session.flash = 'Payment failed: %s' % e.user_message
            redirect(URL('checkout'))
        except Exception as e:
            # Other error
            session.flash = 'Payment error occurred'
            redirect(URL('checkout'))
    else:
        redirect(URL('checkout'))
...
def create_payment_intent():
    ...
    try:
        intent = stripe.PaymentIntent.create(
            amount=int(request.vars.amount),
            currency=request.vars.currency,
            metadata={'order_id': request.vars.order_id}
        )
...
def checkout():
    ...
    if request.env.request_method == 'POST' and request.body:
        ...
        stripe_token = all_forms_data.get('stripe_token')
        ...
        if s_accepted and b_accepted:
            ...
            if stripe_token:
                try:
                    ...
                    return json.dumps(dict(
                        status='error',
                        stripe_error='Too many requests. Please try again later.'
                    ))
                    ...
                    return json.dumps(dict(
                        status='error',
                        stripe_error='Invalid payment information.'
                    ))
                    ...
                    return json.dumps(dict(
                        status='error',
                        stripe_error='Payment authentication failed.'
                    ))
                    ...
                    return json.dumps(dict(
                        status='error',
                        stripe_error='Network error. Please try again.'
                    ))
                except stripe.error.StripeError as e:
                    # DEBUG
                    print("General Stripe error:", e)
                    return json.dumps(dict(
                        status='error',
                    ...
                    return json.dumps(dict(
                        status='error',
                        stripe_error='An unexpected error occured. Please try again.'
                    ))
            else:
                # DEBUG
                print("No stripe token provided")
                return json.dumps(dict(
                    status='error',
                    stripe_error='Payment information is required.'
                ))
